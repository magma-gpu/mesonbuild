...
class ConvertBackend:
    ...
    def get_label(self, platform: HermeticPlatformInstance,
                  custom_instances: T.Set[SelectInstance]) -> T.Set[SelectInstance]:  # fmt: skip
    ...
    def add_rust_bindgen(self, target: ConvertRustBindgen, instance: ConvertInstanceRustBindgen) -> None:  # fmt: skip
        raise NotImplementedError
...
class ConvertTree:
    """A tree structure where each level tracks the ConvertTargets.  There is a
    {BUILD.bazel, Android.bp, ..} file associated with each node of the tree.
    """
...
class ConvertStateTracker:
    ...
    def __init__(self, project_config: ConvertProjectConfig, backend: ConvertBackend):
        ...
        self.current_platform: T.Optional[HermeticPlatformInstance] = None
        self.current_custom_select_instances: T.Optional[T.Set[SelectInstance]] = None
        self.all_platforms: T.Set[HermeticPlatformInstance] = set()
        self.output_dir: str = ''
        self.project_dir: str = ''
        self.backend = backend
        self.targets = ConvertTree()

    def set_current_config(self, platform_info: HermeticPlatformInstance,
                           custom_select_instances: T.Set[SelectInstance]) -> None:  # fmt: skip
        self.current_platform = platform_info
        self.current_custom_select_instances = custom_select_instances

        self.all_platforms.add(platform_info)

    def finish_current_config(self) -> None:
        self.backend.finish_current_config(self)

    def add_python_binary(self, binary: ConvertInstancePythonBinary) -> None:
        if binary.name not in self.targets:
            self.targets[binary.name] = ConvertPythonBinary(binary.name, binary.subdir)

        self.backend.add_python_binary_config(
            T.cast(ConvertPythonBinary, self.targets[binary.name]), binary
        )
    ...
    def add_file_group(self, grp: ConvertInstanceFileGroup) -> None:
        ...
        self.backend.add_file_group_config(
            T.cast(ConvertFileGroup, self.targets[grp.name]),
            grp,
            self.current_platform,
            self.current_custom_select_instances,
        )
    ...
    def add_rust_bindgen(self, rust_bindgen: ConvertInstanceRustBindgen) -> None:
        if rust_bindgen.target_name not in self.targets:
            self.targets[rust_bindgen.target_name] = ConvertRustBindgen(
                rust_bindgen.target_name, rust_bindgen.subdir
            )

        self.backend.add_rust_bindgen(
            T.cast(ConvertRustBindgen, self.targets[rust_bindgen.target_name]), rust_bindgen
        )

    def add_static_library(self, lib: ConvertInstanceStaticLibrary) -> None:
        if lib.name not in self.targets:
            self.targets[lib.name] = ConvertStaticLibrary(lib.name, lib.subdir, lib.rust_abi)
    ...
    def add_shared_library(self, lib: ConvertInstanceSharedLibrary) -> None:
        if lib.name not in self.targets:
            self.targets[lib.name] = ConvertSharedLibrary(lib.name, lib.subdir, lib.rust_abi)
        self.backend.add_build_target_config(
    ...
    def add_executable(self, exe: ConvertInstanceExecutable) -> None:
        if exe.name not in self.targets:
            self.targets[exe.name] = ConvertExecutable(exe.name, exe.subdir, exe.rust_abi)
        self.backend.add_build_target_config(
            T.cast(ConvertExecutable, self.targets[exe.name]),
            exe,
            self.current_platform,
            self.current_custom_select_instances,
        )
    ...
    def finish(self) -> None:
        ...
        for platform in self.all_platforms:
            all_os_selects.add(self.backend.get_os_info(platform, MachineChoice.HOST))
            all_arch_selects.add(self.backend.get_arch_info(platform, MachineChoice.HOST))
